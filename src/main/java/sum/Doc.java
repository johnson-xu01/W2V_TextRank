...
public class Doc {
    ...
    //calculate the similarity of two sentence with word2vec
    void calcSim_W2V() {
        ...
        for (int i = 0; i < snum; i++) {
            ...
            for (int j = 0; j < snum; j++) {
                if (i == j) {
                    ...
                    sim[i][j] = word2vec.sentenceSimilarity(sen.get(i), sen.get(j));
                }
                sumISim += sim[i][j];
            }
            for (int j = 0; j < snum; ++j) {
    ...
    // author: xuyiqiang
    ArrayList<Integer> pickSentence(double[] score) {
        Tuple<Double, Integer>[] score_indexes = new Tuple[snum];
        for (int i = 0; i < snum; i++) {
            Tuple temp = new Tuple(score[i], i);
            score_indexes[i] = temp;
        }
        Arrays.sort(score_indexes, new Comparator<Tuple<Double, Integer>>() {
            @Override
